...
: <<'COMMENT'
export CONFIG=./src/examples/weight_suggest_sweep.json
export OUTDIR=./output/ws_high_npu_bw
export MODE=grid
export OBJECTIVE=total
export ALGOS="Bifocal"
export COMBO_WORKERS=24
export PARALLEL_GROUP_KEYS="pim_weight_load_overlap_ratio,weight_load_compute_overlap_ratio,model,prefill_len,decode_len,batch"
...
SWEEP_SCRIPT="${SWEEP_SCRIPT:-./src/sweep_weight_suggest_params.py}"
...
PIM_WEIGHT_LOAD_OVERLAP_RATIOS_STR="${PIM_WEIGHT_LOAD_OVERLAP_RATIOS:-}"
WEIGHT_LOAD_COMPUTE_OVERLAP_RATIOS_STR="${WEIGHT_LOAD_COMPUTE_OVERLAP_RATIOS:-}"
...
declare -a TP_QKV_ARR=() TP_FFN_ARR=() ALGOS_ARR=() NPU_BACKENDS_ARR=() PIM_WEIGHT_LOAD_OVERLAP_RATIOS_ARR=() WEIGHT_LOAD_COMPUTE_OVERLAP_RATIOS_ARR=()
...
read -r -a PIM_WEIGHT_LOAD_OVERLAP_RATIOS_ARR <<< "${PIM_WEIGHT_LOAD_OVERLAP_RATIOS_STR}"
read -r -a WEIGHT_LOAD_COMPUTE_OVERLAP_RATIOS_ARR <<< "${WEIGHT_LOAD_COMPUTE_OVERLAP_RATIOS_STR}"
...
if (( ${#PIM_WEIGHT_LOAD_OVERLAP_RATIOS_ARR[@]} )); then
  CMD_COMMON+=(--pim-weight-load-overlap-ratio "${PIM_WEIGHT_LOAD_OVERLAP_RATIOS_ARR[@]}")
fi
if (( ${#WEIGHT_LOAD_COMPUTE_OVERLAP_RATIOS_ARR[@]} )); then
  CMD_COMMON+=(--weight-load-compute-overlap-ratio "${WEIGHT_LOAD_COMPUTE_OVERLAP_RATIOS_ARR[@]}")
fi
...
echo "[info] PIM_WEIGHT_LOAD_OVERLAP_RATIOS=${PIM_WEIGHT_LOAD_OVERLAP_RATIOS_STR}"
echo "[info] WEIGHT_LOAD_COMPUTE_OVERLAP_RATIOS=${WEIGHT_LOAD_COMPUTE_OVERLAP_RATIOS_STR}"

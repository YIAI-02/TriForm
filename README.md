...
For generating the results in `./figs/paper_supplementary/sec_6_2`, the simulation time for a single case ranges from 5 minutes to 5 hours. We used `2 × Intel Xeon Platinum 8358` CPU, and the script `./commands/sweep_models_npu_aim_evaluate.sh` can execute 64 cases in parallel. The total simulation time for all cases is within 24 hours.

For generating the results in `./figs/paper_supplementary/sec_6_4`, the script `./commands/run_hpc_sweep_weight_suggest.sh` can execute 75 cases in parallel, with a total simulation time under 3 hours.

...
The browser demo visualizes **simulated CSV traces** generated by the framework. The browser timeline primarily consumes exported operator traces:
...
We store bundled demo cases under [`experiment/demo/examples/`]. Each case should have its own subdirectory containing a baseline operator trace and a heuristic/Bifocal operator trace, for example:
...
Register the case in [`experiment/demo/examples/manifest.json`](./experiment/demo/examples/manifest.json). Once registered, it appears in the **Bundled example** dropdown in `index.html` and can be loaded without manually selecting files. The full storage convention and manifest schema are documented in the [demo trace examples guide](./experiment/demo/examples/README.md).

...
def _comparison_cfg_without_prior_export(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    out.pop("hetinfer_atlas_timings", None)
    out.pop("hetinfer_atlas_manifest_out", None)
    return out
...
def run(cfg: Dict):
    ...
    ) and cfg.get('scheduler_seed') is None:
        raise ValueError(
    ...
    def _evaluate_map(fmt_map_eval: Dict[str, str], *, tag: str) -> Tuple[float, float, float, Any, Any, Dict, Any]:
        ...
        if SchedCls is BifocalScheduler:
            scheduler_kwargs["rand_seed"] = cfg.get("scheduler_seed")
        sched = SchedCls(
            cluster,
            cost,
            label,
    ...
    # layout and capture the completed placement/static timing manifest.
    # Existing AE summaries/timelines remain unchanged.
    ...
    if (
        ...
        if algo_name != 'Bifocal':
            raise ValueError(
                'Het-Infer prior/ATLAS manifest export in weight-suggest '
                'requires algo=Bifocal'
            )
        (_, _, _, _, _, _, prior_sched) = _evaluate_map(
            dict(best_map or {}), tag='hetinfer_prior_best_layout'
        )
        snapshots = list(prior_sched.export_hetinfer_prior_snapshots() or [])
        if not snapshots:
            raise RuntimeError('Bifocal produced no completed static-prior snapshots')
        output_tag = _build_tag(cfg)
        manifest_path = (
            None
        ...
        # overwrite enabled.  Canonical path comparison also catches aliases
        # through relative components and existing symlinks.
        atlas = (
            None

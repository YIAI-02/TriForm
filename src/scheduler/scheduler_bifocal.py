...
class BifocalScheduler(HEFTScheduler):
    ...
    def schedule(self, g: TaskGraph, phase: str) -> List[ScheduledTask]:
        ...
        # Count only successfully published snapshots so failed/disabled runs
        # cannot change the stable ids used by the ATLAS two-stage workflow.
        ...
        def _candidates_for(nid: str, node: TaskNode) -> Tuple[List[DeviceSpec], bool]:
            ...
            # This block intentionally preserves the pre-export scheduler
            # candidate construction byte-for-byte in behavior.  The export
            # finalizer runs only after placement has completed.
            ...
            if is_kv_write:
                ...
                if pinned_dev is not None and (
                    not self._node_allowed_on(node, pinned_dev)
                ):
                    pinned_dev = None
        ...
        def _best_assignment_for(nid: str) -> Tuple[
            ...
            """Return the best placement without publishing score internals."""
            node = g.nodes[nid]

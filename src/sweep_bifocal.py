...
# sweep_bifocal.py
'''
python3 sweep_bifocal.py --mode grid --gamma 0 0.2 0.4 0.6 --lambda_ 0 4 8 12 --eta 0 50 100 200 --objective total --outdir ./output/sweep_bifocal --resume
'''
import argparse
import csv
...
def parse_bifocal_metrics(log_text: str) -> Optional[Dict[str, float]]:
    """
    Parse last occurrence of algo:Bifocal line:
...
def main():
    ap = argparse.ArgumentParser(description="Grid/random sweep for Bifocal total time")
    ...
    ap.add_argument("--workdir", default=str(SCRIPT_DIR), help="working directory")
    ...
    ap.add_argument("--outdir", default=str(PROJECT_ROOT / "output" / "sweep_bifocal"), help="output directory for logs/results")
    ...
    try:
        for (g, lam, eta) in combos:
            ...
            for rep in range(1, int(args.repeat) + 1):
                ...
                print(f"[ok] Bifocal prefill={metrics['prefill']:.4f} decode={metrics['decode']:.4f} total={metrics['total']:.4f} -> objective={obj:.4f}")
                row = {
                    "timestamp": ts,
                    "run_id": run_id,
                    "repeat_idx": rep,

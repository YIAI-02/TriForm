...
# ===== 算法排序（按你的要求）=====
PREFERRED_ORDER = ["pd", "attn_on_pim", "weights_on_pim", "facil", "attacc", "ianus"]
...
def _order_policies(policies: Sequence[str]) -> List[str]:
    ...
    # heft 放最后（兼容你原脚本习惯）
    hefts = [p for p in remaining if _canonical_algo(p) == "heft"]
    remaining = [p for p in remaining if _canonical_algo(p) != "heft"]

    return ordered + remaining + hefts

...
One command, one file:
  python analyze_heft_speedup_all_in_one.py \
    --root /path/to/algorithms/output/... \
    --out  /path/to/analysis_out \
    --baseline ALL
...
def infer_hardware(cfg: Dict[str, Any], file_path: str) -> Dict[str, Any]:
    ...
    # st inference from path (any segment containing st\d+)
    st = None
...
def plot_mean_speedup_batch_prefill_decode(p: Path):
    ...
    for b in sorted(df["batch"].unique()):
        ...
        if pv.empty:
            continue
        arr = np.log2(pv.values.astype(float))
        ...
        savefig(out_dir / f"heatmap_speedup_batch={int(b)}.png")

def plot_best_algo_counts(p: Path, out_dir: Path):
    if not MPL_OK:
        return
    df = read_csv_robust(p)
    if "best_algo_total" not in df.columns or "count" not in df.columns:
        return
    ...
    if df.empty:
        return
    plt.figure(figsize=(10, max(4, 0.35 * len(df))))
    plt.barh(df["best_algo_total"].astype(str), df["count"].values, color=PALETTE[0])
    plt.title("Winner counts: best_algo_total")
    plt.xlabel("count")
    savefig(out_dir / "_viz" / "best_algo_total_counts.png")
...
def dispatch(csv_path: Path, root_out: Path):
    ...
    elif name == "best_algo_total_counts.csv":
        plot_best_algo_counts(csv_path, root_out)
    elif name.startswith("best_algo_total_assoc_cramersv"):
        plot_best_algo_assoc_cramersv(csv_path, root_out)
    elif name.startswith("best_algo_total_assoc_mi"):
...
def analyze(root: Path, out_dir: Path, baseline_arg: str, topk: int, no_ml: bool, no_plots: bool,
            mi_bins: int, assoc_bins: int, dcor_max_n: int, random_state: int):
    ...
    if best_rows:
        ...
        counts = best_df["best_algo_total"].astype(str).value_counts().rename_axis("best_algo_total").reset_index(name="count")
        ...
        # Binned (recommended)
        assoc_b = best_algo_association_binned(best_df, "best_algo_total", assoc_features, bins=assoc_bins)
        assoc_b.to_csv(out_dir / "best_algo_total_assoc_cramersv_binned.csv", index=False)
        print(f"[OK] wrote: {out_dir/'best_algo_total_assoc_cramersv_binned.csv'}")

        mi_b = best_algo_mi_binned(best_df, "best_algo_total", assoc_features, bins=assoc_bins)
        mi_b.to_csv(out_dir / "best_algo_total_assoc_mi_binned.csv", index=False)
        print(f"[OK] wrote: {out_dir/'best_algo_total_assoc_mi_binned.csv'}")

        cond_b = best_algo_conditional_binned_long(best_df, "best_algo_total", assoc_features, bins=assoc_bins)
        cond_b.to_csv(out_dir / "best_algo_total_conditional_binned_long.csv", index=False)
        print(f"[OK] wrote: {out_dir/'best_algo_total_conditional_binned_long.csv'}")

...
static inline uint32_t GetEnvU32(const char* k, uint32_t defv) {
    const char* s = std::getenv(k); if (!s || !*s) return defv;
    char* e=nullptr; unsigned long v = std::strtoul(s, &e, 10);
    return (e==s) ? defv : static_cast<uint32_t>(v);
}
...
{
    // 1) 从环境变量获取维度（run.sh 会设置 LN_A/LN_R）
    const uint32_t aLen = GetEnvU32("LN_A", ALENGTH);
    const uint32_t rLen = GetEnvU32("LN_R", RLENGTH);
    const uint32_t rPadEnv = GetEnvU32("LN_RPAD", 0);
    const uint32_t rLenWithPad = rPadEnv ? rPadEnv : AlignUp(rLen, 32);  // 缺省 32 对齐

    uint32_t blockDim = BLOCK_DIM;
    size_t workspaceSize = WORKSPACE_SIZE * sizeof(float);
    size_t xSize = static_cast<size_t>(aLen) * rLenWithPad * sizeof(float);
    size_t gammaSize = static_cast<size_t>(rLenWithPad) * sizeof(float);
    size_t betaSize = static_cast<size_t>(rLenWithPad) * sizeof(float);
    size_t outputSize = static_cast<size_t>(aLen) * rLenWithPad * sizeof(float);
    size_t meanSize = static_cast<size_t>(aLen) * sizeof(float);
    size_t rstdSize = static_cast<size_t>(aLen) * sizeof(float);
    size_t tilingFileSize = sizeof(optiling::LayernormCustomTilingData);
    bool goldenResult = true;
    const bool noIO = (std::getenv("LN_NO_IO") != nullptr) && (std::string(std::getenv("LN_NO_IO")) != "0");
    uint8_t *tilingBuf = GenerateTiling(aLen, rLen, rLenWithPad);
    ...
    if (noIO) {
        for (size_t i = 0; i < xSize/sizeof(float); ++i)
            reinterpret_cast<float*>(inputXHost)[i] = static_cast<float>((i % 13) - 6) * 0.01f;
        std::fill_n(reinterpret_cast<float*>(gammaHost), gammaSize/sizeof(float), 1.0f);
        std::fill_n(reinterpret_cast<float*>(betaHost),  betaSize/sizeof(float),  0.0f);
    } else {
    ...
    if (!noIO) {
        ...
        CHECK_ACL(aclrtMemcpy(rstdHost, rstdSize, rstdDevice, rstdSize, ACL_MEMCPY_DEVICE_TO_HOST));
    ...
    free(tilingBuf);
    if (!noIO) {
        if (goldenResult) printf("test pass!\n");
        else printf("test failed!\n");
    } else {
        printf("done: a=%u, r=%u (pad=%u)\n", aLen, rLen, rLenWithPad);
    }
    return 0;
}
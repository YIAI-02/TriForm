# ===== measurements/pim/02_run_ramulator.py  （替换整个文件）=====
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
FEATURE_SPECS = [
    # name, has_opsize, regex patterns
    ...
    # （可选）PNM 侧：如 trace 中出现，则统计到
    ("EXP",         True,  [r"^(?:PNM|AiM)\s+EXP\s+(\d+)"]),
    ("RED",         True,  [r"^(?:PNM|AiM)\s+RED\s+(\d+)"]),
    ("ACC",         True,  [r"^(?:PNM|AiM)\s+ACC\s+(\d+)"]),
    ("RISCV",       False, [r"^(?:PNM|AiM)\s+RISCV\b"]),
]
...
DEFAULT_PATTERNS = [ r"memory_system_cycles:\s*([0-9]+)" ]
            ...
            for name in FEATURE_NAMES:
                for pat in FEATURE_PATTERNS[name]:
                    ...
                    if FEATURE_HAS_SIZE[name] and m.lastindex:
                        try: counts[name][1] += int(m.group(1))
                        except: pass
                    break
...
def parse_meta(trace_path: Path) -> Dict[str, str | int | None]:
    ...
    if meta["with_af"] is None:
        meta["with_af"] = 1 if "_withaf" in trace_path.name else 0
    # seqlen / vector_dim / matrix_col 兜底
    name = trace_path.name
    m = re.search(r"_seq(\d+)_", name)
    if m and meta["seqlen"] is None: meta["seqlen"] = int(m.group(1))
    m = re.search(r"_vec(\d+)_", name)
    if m and meta["vector_dim"] is None: meta["vector_dim"] = int(m.group(1))
    m = re.search(r"_col(\d+)_", name)
    if m and meta["matrix_col"] is None: meta["matrix_col"] = int(m.group(1))
    # dim / heads
    m = re.search(r"_dim(\d+)_h(\d+)_hk(\d+)", name)
    if m:
        meta["dim"] = meta["dim"] or int(m.group(1))
        meta["n_heads"] = meta["n_heads"] or int(m.group(2))
        meta["n_kv_heads"] = meta["n_kv_heads"] or int(m.group(3))
    return meta

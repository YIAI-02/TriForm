#!/usr/bin/env python3
# run_hefthint_one.py
"""Run ONE hefthint config (gamma/lambda/eta) once.
...
Typical usage (inside an isolated workdir / per-job checkout):
  python3 run_hefthint_one.py \
    --config config.py \
    --script ./command_single.sh \
    --workdir . \
    --gamma 0.2 --lambda_ 2 --eta 0 \
    --objective total \
    --outdir ./output/sweep_hefthint_parallel
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Run ONE hefthint config once (for parallel launching).")
    ...
    ap.add_argument("--workdir", default=".", help="working directory (where command script runs)")
    ...
    ap.add_argument("--outdir", default="./output/sweep_hefthint_parallel", help="output root dir")
    ...
    script_path = workdir / args.script
    ...
    try:
        ...
        if metrics is None:
            ...
            print(
                f"[ok] hefthint prefill={metrics['prefill']:.4f} "

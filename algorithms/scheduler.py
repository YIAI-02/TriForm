...
class HEFTScheduler:
    ...
    def _earliest_finish_hybrid(self, g: TaskGraph, nid: str, phase: str, commit: bool) -> Optional[Dict[str, object]]:
        ...
        if start_npu <= start_pim:
            lead_start = start_npu
            tail_start = start_pim
            r_lead = rN
        else:
            lead_start = start_pim
            tail_start = start_npu
            r_lead = rP
        lead_interval = max(0.0, tail_start - lead_start)
        work_done = r_lead * lead_interval
        logger.debug(str(f'tN: {tN}, tP: {tP}'))
        agg = rN + rP
        ...
        return {'mode': 'HYBRID', 'start_npu': start_npu, 'start_pim': start_pim, 'finish': finish, 'npu': npu_dev, 'pim': pim_dev, 'out_dev': out_dev}

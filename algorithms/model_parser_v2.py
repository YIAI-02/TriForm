...
def build_graph(cfg: Dict[str, Any], pim_shards: int = None, split_by: str = None):
    ...
    # NOTE: All tensor-parallel / sharding controls have been removed from
    # model_definition. We keep `split_by` only as a *KV placement* policy knob
    # (used by the memory planner / scheduler), not for graph construction.
    part_dim = str(
        split_by if split_by is not None else (cfg.get('split_by', 'head') or 'head')
    ).strip().lower()
    ...
    if bool(cfg.get("dump_graph", False)) or bool(cfg.get("dump_task_graph", False)):
        ...
        tag = str(
            ...
            or f"{family}_{variant}_{part_dim}_"

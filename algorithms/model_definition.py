...
class MixtralDef:
    ...
    def build(self, shape: ModelShape, dtype_bytes: int) -> TaskGraph:
        ...
        for l in range(shape.layer_num):
            ...
            g.add_node(TaskNode(nid_O, "O", flops=0.0,
                                weight_id=f"L{l}_WO",
                                weight_size=o_in_dim * dim * dtype_bytes,
                                attrs=dict(base_attr),
                                allowed=get_op_allowed("O")))
            nid_Add1 = f"L_{l}_Add1"
            g.add_node(TaskNode(nid_Add1,"Add",flops=0.0,attrs=dict(base_attr),allowed=get_op_allowed("Add")))
            ...
            # LN2 for FFN
            g.add_edge(nid_LN1, nid_Q);g.add_edge(nid_LN1, nid_K);g.add_edge(nid_LN1, nid_V)
            g.add_edge(nid_Q, nid_QK);g.add_edge(nid_K, nid_QK);g.add_edge(nid_KR, nid_QK)
            g.add_edge(nid_QK, nid_SO);g.add_edge(nid_SO, nid_SV)
            g.add_edge(nid_V, nid_SV);g.add_edge(nid_VR, nid_SV)
            g.add_edge(nid_SV, nid_O)
            ...
            nid_LN2 = f"L{l}_LN2"
            g.add_node(TaskNode(nid_LN2, "LN", flops=0.0,
                                attrs=dict(base_attr),
                                allowed=get_op_allowed("LN")))
            ...
            for e in range(active_experts):
                ...
                g.add_node(TaskNode(
                    nid_ACT, "SwiGLU", flops=0.0,
                    attrs=dict(expert_attr),
                    allowed=get_op_allowed("SwiGLU"),
                ))
                g.add_node(TaskNode(
                    nid_W2, "FFN_W2", flops=0.0,

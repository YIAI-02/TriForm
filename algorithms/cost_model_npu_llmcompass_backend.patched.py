...
def llmcompass_register_device_obj(device_key: str, device_obj: Any) -> None:
    k = _llmcompass_normalize_device_key(device_key)
    _LLMCOMPASS_DEVICE_OVERRIDE[k] = device_obj
    _LLMCOMPASS_DEVICE_CACHE.pop(k, None)

def llmcompass_register_device_json(device_key: str, json_path: Path) -> None:
    json_path = Path(json_path)
    if not json_path.exists():
        raise FileNotFoundError(f'LLMCompass device json not found: {json_path}')
    ...
    try:
        ...
        cfg_dict = _json.loads(json_path.read_text(encoding='utf-8'))
    except Exception:
        cfg_dict = None

    dev_obj = None
    DeviceCls = getattr(device_mod, 'Device', None)
    if DeviceCls is not None:
        # (1) Device('/path/to/config.json')
        try:
            dev_obj = DeviceCls(str(json_path))
        except Exception:
            dev_obj = None
        # (2) Device(cfg_dict)
        if dev_obj is None and cfg_dict is not None:
            try:
                dev_obj = DeviceCls(cfg_dict)
            except Exception:
                dev_obj = None
    ...
    if dev_obj is None:
        ...
        for fname in (
            'device_from_json',
            'load_device_from_json',
            'create_device_from_json',
            'from_json',
            'make_device_from_json',
        ):
            fn = getattr(device_mod, fname, None)
            if callable(fn):
                # (a) path
                try:
                    dev_obj = fn(str(json_path))
                    break
                except Exception:
                    dev_obj = None
                # (b) dict
    ...
    if dev_obj is None:
        raise RuntimeError(
            f'Cannot create LLMCompass Device from json: {json_path}. '
            'Please adapt llmcompass_register_device_json() to your LLMCompass version.'
        )

    llmcompass_register_device_obj(device_key, dev_obj)

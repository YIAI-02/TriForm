...
class CostModel:
    ...
    def _opt_phase_lookup(self, d: Dict[str, Any], key: str, phase: str, default=None):
        """Lookup a possibly per-phase value."""
    ...
    def estimate_activation_bytes(self, node, batch: int, seq_len: int, phase: str):
        ...
        def moe_token_fraction() -> float:
            ...
            return min(1.0, base * imbalance)
        if name == 'LN' and D > 0:
            elems = dens_store * (b * active_tokens * D)
            return (to_bytes(elems), to_bytes(elems))
        if name == 'Q' and D > 0:
            out_dim = q_dim if q_dim > 0 else D
            return (
                to_bytes(dens_store * (b * active_tokens * D)),
                to_bytes(dens_store * (b * active_tokens * out_dim)),
            )
        ...
        if name == 'O' and D > 0:
            ...
            return (
                to_bytes(dens_store * (b * active_tokens * inp_dim)),
                to_bytes(dens_store * (b * active_tokens * D)),
            )
        ...
        if name in ('SWIGLU', 'SILU_GLU') and Hf > 0:
            return (
                to_bytes(dens_store * (b * active_tokens * (2 * Hf))),
                to_bytes(dens_store * (b * active_tokens * Hf)),
            )
        if name in ('GELU', 'RELU'):
            width = Hf if Hf > 0 else D
            return (to_bytes(dens_store * (b * active_tokens * width)), to_bytes(dens_store * (b * active_tokens * width)))
        if name == 'ADD' and D > 0:
            read_elems = dens_store * (b * active_tokens * D * 2)
            write_elems = dens_store * (b * active_tokens * D)
            return (to_bytes(read_elems), to_bytes(write_elems))
        if name in ('IDENTITY',):
            elems = dens_store * (b * active_tokens * D)
            return (to_bytes(elems), to_bytes(elems))
        if name == 'QK' and qh > 0 and (hd > 0):
            q_read = dens_store * (b * active_tokens * q_dim)
        ...
        if name == 'SV' and qh > 0 and (hd > 0):
            ...
            out_elems = dens_store * (b * qh * active_tokens * hd)
    ...
    def estimate_kv_cache_read_bytes(self, node, batch: int, seq_len: int, phase: str) -> int:
        ...
        # Attention sparsity-aware KV history length (decode)
    ...
    def node_device_cost(self, node: TaskNode, dev: DeviceSpec, label: PlanLabel, batch: int, seq_len: int, phase: str) -> float:
        ...
        if dev.type == 'npu':
            ...
            if op_key == 'softmax':
                ...
                # If attention sparsity is enabled, approximate the per-row softmax width via the effective attention pairs.
                ...
                if pat not in ('dense', 'none', 'off', 'disabled'):
                    pairs = int(self._attention_pairs(node, seq_len, phase, causal=causal))
                    if phase == 'prefill':
        ...
        if dev.type == 'pim':
            ...
            return float(compute_time + mem_time) * time_scale
        return 0.0

...
def _partition_ranges_strict(total: int, parts: int) -> List[Tuple[int, int]]:
    """Return *equal-sized* contiguous (start, count) ranges.
    """
    total = int(max(0, total))
    if total <= 0:
        return []
    ...
    if parts <= 0:
        parts = 1
    if parts > total:
        raise ValueError(f"Cannot shard total={total} into parts={parts}: parts > total")
    if (total % parts) != 0:
        raise ValueError(f"Cannot evenly shard total={total} into parts={parts}: not divisible")

    cnt = total // parts
    out: List[Tuple[int, int]] = []
    start = 0
    for _ in range(parts):
        out.append((int(start), int(cnt)))
        start += cnt
    return out


def _partition_q_ranges_from_kv_ranges_strict(
    kv_ranges: List[Tuple[int, int]],
    *,
    q_heads: int,
    kv_heads: int,
) -> List[Tuple[int, int]]:
    """Strict GQA head-range mapping.

    Requirements:
      - kv_heads <= q_heads
      - q_heads % kv_heads == 0  (integer q-per-kv group)

    For each KV-head range, we assign the *exact* corresponding Q-head range
    so that every Q shard consumes only KV heads in its shard.
    """
    q_heads = int(q_heads or 0)
    kv_heads = int(kv_heads or 0)
    if q_heads <= 0 or kv_heads <= 0:
        return []
    if kv_heads > q_heads:
        raise ValueError(f"Invalid GQA config: kv_heads({kv_heads}) > q_heads({q_heads})")
    if (q_heads % kv_heads) != 0:
        raise ValueError(
            f"Invalid GQA config: q_heads({q_heads}) must be divisible by kv_heads({kv_heads})"
        )
    ...
    for kv_st, kv_cnt in kv_ranges:
        kv_st = int(kv_st)
        kv_cnt = int(kv_cnt)
        q_st = int(kv_st * q_per_kv)
        q_cnt = int(kv_cnt * q_per_kv)
        out.append((q_st, q_cnt))

    # Safety: ensure the last range does not overflow.
    if out:
        last_st, last_cnt = out[-1]
        if int(last_st + last_cnt) > int(q_heads):
            raise ValueError(
                f"Derived q-ranges overflow: last_end={last_st+last_cnt} > q_heads={q_heads}"
            )
    return out
...
def add_llama_block_split_by_heads(g: TaskGraph, l: int, shape: ModelShape, dtype_bytes: int):
    ...
    # ------------------------------------------------------------------
    # Strict TP head sharding (NO uneven shards, NO KV replication)
    # ------------------------------------------------------------------
    kv_compute_shards = int(_effective_shards(shape, for_kv=True))
    tp = int(max(1, kv_compute_shards))
    if kvh > 0:
        tp = int(min(tp, int(kvh)))
    if qh > 0:
        tp = int(min(tp, int(qh)))

    # Enforce strict divisibility.
    if kvh > 0:
        kv_ranges_compute = _partition_ranges_strict(int(kvh), int(tp))
    else:
        kv_ranges_compute = []
    kv_ranges_cache = list(kv_ranges_compute)
    # GQA strict mapping (also enforces qh % kvh == 0 and kvh <= qh)
    q_ranges = _partition_q_ranges_from_kv_ranges_strict(kv_ranges_compute, q_heads=qh, kv_heads=kvh)
    q_shards = int(len(q_ranges))
    if q_shards != int(len(kv_ranges_compute)):
        raise ValueError(
            f"[TP] Q/KV shard count mismatch: q_shards={q_shards} vs kv_shards={len(kv_ranges_compute)}"
        )

    # KV-cache shards must match KV compute shards.
    kv_cache_shards = int(len(kv_ranges_cache))

    # FFN TP degree: default align to attention TP (strict, equal-sized shards).
    ffn_shards = int(max(1, min(int(tp), int(ffn) if int(ffn) > 0 else 1)))
    if int(ffn) > 0 and (int(ffn) % int(ffn_shards)) != 0:
        raise ValueError(
            f"[TP] ffn_dim({ffn}) must be divisible by ffn_shards({ffn_shards}) for strict TP"
        )
    ...
    # ---- Q + attention shards (STRICTLY aligned to KV shards) ----
    o_nodes: List[str] = []
    for sid, ((q_st, q_cnt), (kv_st, kv_cnt)) in enumerate(zip(q_ranges, kv_ranges_compute)):
        q_dim_s = q_cnt * hd
        ...
        # This shard's KV heads are exactly kv_st..kv_st+kv_cnt-1.
        kv_head_ids = list(range(int(kv_st), int(kv_st + kv_cnt)))
        attr_attn = dict(attr_q)
        attr_attn.update({
            "kv_heads": int(kv_cnt),
            "n_kv_heads": int(kv_cnt),
            "kv_dim": int(int(kv_cnt) * int(hd)),
            "kv_head_ids": kv_head_ids,
            "kv_head_id": int(kv_st) if kv_head_ids else 0,
            "kv_shard_ids": [int(sid)],
        })
        ...
        # Local KV only.
        g.add_edge(k_nodes[int(sid)], nid_QK)
        g.add_edge(v_nodes[int(sid)], nid_SV)
    ...
    # Merge O shards -> full attention output (row-parallel WO => ALLREDUCE-SUM).
    if len(o_nodes) <= 1:
        nid_Omerge = o_nodes[0] if o_nodes else None
    else:
    ...
    # FFN2 (down-proj) uses input-sharded matmul => SUM-reduction merge.
    if len(w2_nodes) <= 1:
        nid_FFNmerge = w2_nodes[0] if w2_nodes else None
    else:
...
def add_mpt_block_split_by_heads(g: TaskGraph, l: int, shape: ModelShape, dtype_bytes: int):
    ...
    kv_ranges_compute = _partition_ranges_strict(int(kvh), int(tp)) if int(kvh) > 0 else []
    kv_ranges_cache = list(kv_ranges_compute)

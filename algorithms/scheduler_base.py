...
class SchedulerBase:
    ...
    def _weight_trace_extra_from_profile(self, prof: Optional[Mapping[str, Any]]) -> Dict[str, Any]:
        ...
        if not wid:
            return {}
        return {
            'weight_host_storage_fmt': str(prof.get('host_storage_fmt', '') or ''),
            'weight_host_src_fmt': str(prof.get('host_src_fmt', '') or ''),
            ...
            'weight_queue_wait_s': float(prof.get('queue_wait_s', 0.0) or 0.0),
            'weight_load_total_s': float(prof.get('load_total_s', 0.0) or 0.0),
            'weight_load_active_s': float(prof.get('load_active_s', 0.0) or 0.0),
            'weight_load_comm_s': float(prof.get('load_comm_s', 0.0) or 0.0),
            'weight_load_l1_s': float(prof.get('load_l1_s', 0.0) or 0.0),
            'weight_load_l2_s': float(prof.get('load_l2_s', 0.0) or 0.0),
            'weight_load_l2_write_only_s': float(prof.get('load_l2_write_only_s', 0.0) or 0.0),
            'weight_load_l2_pack_only_est_s': float(prof.get('load_l2_pack_only_est_s', 0.0) or 0.0),
            'weight_load_l1_l2_overlap_ratio': float(prof.get('load_l1_l2_overlap_ratio', 0.0) or 0.0),
            'weight_compute_total_s': float(prof.get('compute_total_s', 0.0) or 0.0),
            'weight_compute_backend': str(prof.get('compute_backend', '') or ''),
            'weight_compute_rule': str(prof.get('compute_rule', '') or ''),
            'weight_b1_s': float(prof.get('b1_s', 0.0) or 0.0),
            'weight_b2_s': float(prof.get('b2_s', 0.0) or 0.0),
            'weight_launch_overhead_s': float(prof.get('launch_overhead_s', 0.0) or 0.0),
            'weight_load_compute_overlap_ratio': float(prof.get('lc_overlap_ratio', 0.0) or 0.0),
            'weight_load_compute_overlap_saved_s': float(prof.get('lc_overlap_saved_s', 0.0) or 0.0),
        }
...
# ?
    ...
    def _earliest_finish_on_device(
        ...
        if self._node_weight_id(node) and int(self._node_weight_size(node) or 0) > 0:
            ...
            self._set_last_op_trace_extra(dict(weight_extra))
        else:

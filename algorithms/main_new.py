    ...
    *,
    ...
    if pim_rr:
        if is_head:
            ...
            for n in graph.nodes.values():
                if str(getattr(n, "name", "") or "").upper() != "K_WRITE":
                    continue
                attrs = getattr(n, "attrs", None)
                if not isinstance(attrs, dict):
                    continue
                sid = attrs.get("kv_shard")
                if sid is None:
                    sid = attrs.get("head_shard")
                if sid is None:
                    sid = attrs.get("pim_target_idx")
                try:
                    sid_i = int(sid)
                except Exception:
                    continue
                ...
                if not isinstance(kv_head_ids, (list, tuple, set)):
                    # fallback: reconstruct from (start, count)
                    try:
                        st = int(attrs.get("kv_head_start"))
                        cnt = int(attrs.get("kv_head_count"))
                        kv_head_ids = list(range(st, st + cnt))
                    except Exception:
                        kv_head_ids = None

                if not isinstance(kv_head_ids, (list, tuple, set)) or not kv_head_ids:
                    continue

                if sid_i not in shard_to_heads:
                    shard_to_heads[sid_i] = set()
                shard_to_heads[sid_i].update(int(h) for h in kv_head_ids)

            if not shard_to_heads:
                raise ValueError(
                    "[TP] split_by=head requested but graph contains no head-sharded K_write nodes"
                )

            # Validate: disjoint + complete.
            all_heads: set[int] = set()
            for sid_i, heads in shard_to_heads.items():
                if all_heads.intersection(heads):
                    raise ValueError(
                        f"[TP] KV head replication detected across shards (shard={sid_i})"
                    )
                all_heads.update(heads)
            if len(all_heads) != int(n_kv_heads):
                raise ValueError(
                    f"[TP] KV head coverage mismatch: mapped={len(all_heads)} expected={n_kv_heads}"
                )

            shard_ids_sorted = sorted(shard_to_heads.keys())
            kv_shards = int(len(shard_ids_sorted))
            if kv_shards > len(pim_rr):
                raise ValueError(
                    f"[TP] kv_shards({kv_shards}) exceeds #PIM devices({len(pim_rr)})"
                )

            # Strict: every shard must have equal number of KV heads.
            sizes = sorted({len(shard_to_heads[sid]) for sid in shard_ids_sorted})
            if len(sizes) != 1:
                raise ValueError(
                    f"[TP] n_kv_heads({n_kv_heads}) must be divisible by kv_shards({kv_shards}); got shard_sizes={sizes}"
                )

            # Map shard->PIM and build kv_head_to_pim.
            for sid_i in shard_ids_sorted:
                pname = pim_rr[int(sid_i) % len(pim_rr)].name
                heads = shard_to_heads[sid_i]
                for h in heads:
                    kv_head_to_pim[int(h)] = str(pname)
                kv_bytes_by_pim[str(pname)] = int(
                    kv_bytes_by_pim.get(str(pname), 0) + len(heads) * kv_bytes_per_head * layers
                )
        else:

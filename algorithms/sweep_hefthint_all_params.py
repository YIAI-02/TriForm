#!/usr/bin/env python3
"""
Sweep HEFTHINT-related scheduler knobs by editing config.py in place,
launching an external evaluation shell script, and collecting the
algo:hefthint metrics.

Compared with the old sweep_hefthint.py, this version:
  1) scans ALL current HEFTHINT knobs listed in config.py;
...
def _parse_metrics_from_stdout_table(log_text: str, target_policy: str) -> Optional[Dict[str, Any]]:
    ...
    # algo:hefthint            host          1.2345       2.3456       3.5801
...
def parse_hefthint_metrics(log_text: str, workdir: Path, target_policy: str) -> Optional[Dict[str, Any]]:
...
def main() -> int:
    ...
    backup_path = config_path.with_suffix(config_path.suffix + ".hefthint_all_sweep_bak")
    ...
    try:
        for params in combos:
            ...
            for rep in range(1, int(args.repeat) + 1):
                ...
                metrics = parse_hefthint_metrics(out, workdir=workdir, target_policy=args.target_policy)

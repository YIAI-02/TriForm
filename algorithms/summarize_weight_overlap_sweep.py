...
def _enrich_row(row: Dict[str, Any]) -> Dict[str, Any]:
    out = dict(row)
    for key in ('pim_weight_load_overlap_ratio', 'weight_load_compute_overlap_ratio'):
        ratio = _to_float(row.get(key), default=float('nan'))
...
def main() -> int:
    ap = argparse.ArgumentParser(description='Summarize weight-suggest sweeps over overlap parameters.')
    ap.add_argument('--results-csv', required=True, help='Path to results.csv produced by sweep_weight_suggest_params.py')
    ap.add_argument('--outdir', default='', help='Output directory. Defaults to <results_dir>/overlap_summary')
    ...
    ap.add_argument('--group-by', default='model,prefill_len,decode_len,batch,hardware_json,tp_qkv,tp_ffn,pim_weight_load_overlap_ratio,weight_load_compute_overlap_ratio', help='Comma-separated keys for detailed grouped summary')
    args = ap.parse_args()

    results_csv = Path(args.results_csv).resolve()
    if not results_csv.exists():
        raise SystemExit(f'results.csv not found: {results_csv}')
    outdir = Path(args.outdir).resolve() if str(args.outdir).strip() else (results_csv.parent / 'overlap_summary').resolve()
    ...
    def _sort_component(key_name: str, row: Dict[str, Any]):
        if key_name in ('pim_weight_load_overlap_ratio', 'weight_load_compute_overlap_ratio'):
            return _ratio_sort_key(row.get(key_name, ''))
        return str(row.get(key_name, '') or '')

    group_rows.sort(key=lambda r: tuple(_sort_component(k, r) for k in group_keys))
    ...
    _write_csv(outdir / 'group_summary.csv', group_rows, list(group_keys) + summary_fields)

    pair_groups: Dict[Tuple[str, str], List[Dict[str, Any]]] = defaultdict(list)
    for row in rows:
        pair = (
            _stable_group_value(row, 'pim_weight_load_overlap_ratio'),
            _stable_group_value(row, 'weight_load_compute_overlap_ratio'),
        )
        pair_groups[pair].append(row)

    pair_rows: List[Dict[str, Any]] = []
    best_by_pair: Dict[str, Dict[str, Any]] = {}
    for (pim_ratio, lc_ratio), subrows in pair_groups.items():
        payload = {
            'pim_weight_load_overlap_ratio': pim_ratio,
            'weight_load_compute_overlap_ratio': lc_ratio,
        }
        summary = _summarize_group(subrows, objective_field=args.objective_field)
        payload.update(summary)
        pair_rows.append(payload)
        best_by_pair[f'{pim_ratio}|{lc_ratio}'] = {
            ...
            'best_weight_format_json': summary['best_weight_format_json'],
        }

    pair_rows.sort(key=lambda r: (
        _ratio_sort_key(r.get('pim_weight_load_overlap_ratio', '')),
        _ratio_sort_key(r.get('weight_load_compute_overlap_ratio', '')),
    ))
    _write_csv(
        outdir / 'overlap_pair_summary.csv',
        pair_rows,
        ['pim_weight_load_overlap_ratio', 'weight_load_compute_overlap_ratio'] + summary_fields,
    )
    ...
    summary_json = {
        ...
        'overlap_pair_summary_csv': str(outdir / 'overlap_pair_summary.csv'),
    ...
    print(f'[summary] overlap_pair_summary_csv={outdir / "overlap_pair_summary.csv"}')

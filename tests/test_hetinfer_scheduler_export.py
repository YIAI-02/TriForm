...
class HetInferSchedulerExportTests(unittest.TestCase):
    def _assert_clean_complete_snapshot(
        ...
        for node_id, operator in operator_by_node.items():
            ...
            descriptor = operator["network_metadata"]
            self.assertEqual(
                set(descriptor),
                {
                    "name",
    ...
    def test_pim_collective_service_and_nonresident_routes_use_dops_costs(
        self,
    ) -> None:
        graph = TaskGraph()
    ...
    def test_real_cost_model_primitives_match_hand_calculation(self) -> None:
        ...
        pim_write.assert_called_once_with(expected_write_bytes, pim)
    ...
    def test_strict_pim_trace_uses_native_latency_and_rejects_unsupported_ops(
        self,
    ) -> None:
        cluster = Cluster()
        pim = DeviceSpec(
        ...
        cost = CostModel(
            cluster,
            pim_config_path=SRC_ROOT / "aim_simulator" / "PIM_AiM.json",
            ramulator_config_path=SRC_ROOT / "aim_simulator" / "example.yaml",
            model_dict={},
            pim_fast_mode=False,
            pim_trace_strict=True,
        )
        label = SimpleNamespace(kv_in_pim=True)
        q_proj = TaskNode(
            id="q_proj",
            name="Q_PROJ",
            allowed={"cpu": False, "npu": False, "pim": True},
            attrs={
                "dim": 128,
                "ffn_dim": 512,
                "q_heads": 1,
        ...
        with mock.patch(
            "costmodel_impl.pim_backends._get_pim_latency_via_trace",
            return_value=0.0125,
        ) as native_latency:
            self.assertEqual(
                cost.node_device_cost(q_proj, pim, label, 1, 8, "prefill"),
                0.0125,
            )
        native_latency.assert_called_once()
        ...
        with mock.patch.object(cost, "pim_mem_time", return_value=0.004) as memory_latency:
            self.assertEqual(
                cost.node_device_cost(v_write, pim, label, 1, 8, "prefill"),
                0.004,
            )
        memory_latency.assert_called_once_with(0, 2048, pim)

        unsupported = TaskNode(
            id="kv_write",
            name="KV_WRITE",
            allowed={"cpu": False, "npu": False, "pim": True},
        )
        with self.assertRaisesRegex(RuntimeError, "does not support"):
            cost.node_device_cost(unsupported, pim, label, 1, 8, "prefill")

    def test_cost_model_builder_forwards_native_pim_runtime_settings(self) -> None:
        cluster = Cluster()
        cluster.add_device(
            DeviceSpec(
                name="NPU0",
        ...
        cfg = {
            ...
            "npu_lut_strict": True,
            "pim_fast_mode": False,
            "pim_ramulator_bin": "/tmp/hetinfer-ramulator2",
            "pim_ramulator_timeout_s": 47,
            "pim_trace_strict": True,
        }
        shape = SimpleNamespace(dim=128, n_heads=1, n_kv_heads=1, ffn_dim=512)
        with mock.patch.object(
            kv_policy_module,
            "_make_shared_model_dict",
            return_value={},
        ), mock.patch.object(kv_policy_module, "CostModel") as constructor:
            kv_policy_module._build_cost_model_for_run(
                cfg,
                cluster,
                shape,
                simulation_log_file=Path("/tmp/hetinfer-pim.log"),
            )
        kwargs = constructor.call_args.kwargs
        self.assertIs(kwargs["npu_lut_strict"], True)
        self.assertEqual(kwargs["pim_ramulator_bin"], Path("/tmp/hetinfer-ramulator2"))
        self.assertEqual(kwargs["pim_ramulator_timeout_s"], 47)
        self.assertIs(kwargs["pim_trace_strict"], True)

    def test_strict_npu_lut_rejects_misses_router_and_unknown_ops(self) -> None:
        cluster = Cluster()
        npu = DeviceSpec(
        ...
        label = SimpleNamespace(kv_in_pim=False)
        attrs = {
            "dim": 128,
            "ffn_dim": 512,
            "q_heads": 1,
        ...
        lut_cases = (
            ("SOFTMAX", "_predict_softmax_latency_us_from_lut"),
            ("GELU", "_predict_gelu_latency_us_from_lut"),
            ("RMSNORM", "_predict_layernorm_latency_us_from_lut"),
            ("Q_PROJ", "_predict_mmad_latency_us_from_lut"),
        )
        for op_name, predictor in lut_cases:
            with self.subTest(op_name=op_name), mock.patch(
                f"costmodel_impl.npu_backends.{predictor}",
                return_value=None,
            ):
                node = TaskNode(
                    id=op_name.lower(),
                ...
                with self.assertRaisesRegex(RuntimeError, "strict mode rejected"):
                    cost.node_device_cost(node, npu, label, 1, 8, "prefill")

        for op_name in ("MOE_ROUTER", "MYSTERY_OP"):

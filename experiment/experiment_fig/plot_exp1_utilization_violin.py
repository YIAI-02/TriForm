...
"""Plot utilization and co-utilization statistics from ``*_ops_trace.csv`` and
``*_comms_trace.csv`` files.

Run this script from ``experiment/experiment_fig``. The examples below assume
trace files were generated by the current ``src/main.py evaluate`` flow and
stored under the repository-level ``output/`` directory.

Typical usage
-------------
python3 plot_exp1_utilization_violin.py \
  --search-dir ../../output/evaluate_single_test \
  --model-prefix llama_7b_fp16 \
  --speedup-plot-max 4 \
  --algo-label-map 'Bifocal=Bifocal,HEFT=HEFT,PD=PD,AF=AF,PD+FFN=PD+FFN,PD+Linear=PD+Linear,PD+Attn=PD+Attn' \
...
SCHEDULER_VARIANTS: set[str] = set()
...
PREFERRED_ALGO_ORDER: List[str] = ["PD", "AF", "PD+FFN", "PD+Linear", "PD+Attn", "HEFT", "Bifocal"]
...
def order_algorithms(
    ...
    rest = sorted([a for a in present_norm if a not in set(ordered) and a != "Bifocal"])
    ordered.extend(rest)
    if "Bifocal" in present_norm:
        ordered.append("Bifocal")
    return ordered
...
def build_results(
    *,
    ...
    phase: str = "all",
    include_cpu: bool = False,
    scheduler_pick_by: str = "family_mean",
    weight_stage_util_mode: str = DEFAULT_WEIGHT_STAGE_UTIL_MODE,
    disk_cache_dir: Optional[Path] = None,
    ...
    normal_algos = [a for a in actual_algos if a not in SCHEDULER_VARIANTS]
            ...
            for d in decodes:
                for algo in normal_algos:
                    if TraceKey(run_id=run_id, algo=algo, prefill=p, decode=d) in index:
                        total_jobs += 1
                for variant in sorted(SCHEDULER_VARIANTS):
            ...
            for d in decodes:
                ...
                if variant_results:
                    def _sel_key(r: UtilResult) -> Tuple[float, int, int]:
                        score = util_pick_score(r, mode=scheduler_pick_by)
                        prefer_bifocal = 1 if r.source_algo == "Bifocal" else 0
                        overall = int(round(r.overall_utilization * 1e12))
                        return (score, prefer_bifocal, overall)

                    best = max(variant_results, key=_sel_key)
                    best_for_plot = UtilResult(
                        run_id=best.run_id,
                        source_algo=best.source_algo,
                        plotted_algo="Bifocal",
                    ...
                    results[(run_id, p, d, "Bifocal")] = best_for_plot
...
def lookup_runtime_info(
    *,
    ...
    if mode == "speedup":
        if norm_algo == "Bifocal":
            candidates.append("Bifocal")
        else:
...
def main() -> None:
    ...
    ap.add_argument("--algo-order", type=str, default=None,
                    help="Optional x-axis algorithm order, e.g. PD,AF,PD+FFN,PD+Linear,PD+Attn,HEFT,Bifocal")
    ap.add_argument("--include-algos", type=str, default=None,
                    help="Only plot these algorithms, in the given order, e.g. PD,AF,PD+FFN,PD+Linear,PD+Attn,HEFT,Bifocal")
    ap.add_argument("--exclude-algos", type=str, default=None,
                    help="Algorithms to drop from plotting, e.g. HEFT")
    ap.add_argument("--algo-label-map", type=str, default=None,
                    help="Override x-axis display names with key=value pairs separated by ';'")
    ap.add_argument("--highlight-algo", type=str, default="Bifocal",
                    help="Highlight this algorithm label on the x-axis. Default: Bifocal")
    ...
    ap.add_argument("--scheduler-pick-by", type=str,
                    choices=["family_mean", "overall", "pim", "accel"],
                    default="family_mean",
                    help="Selection policy when multiple scheduler traces match the same case.")
    ...
    results = build_results(
        ...
        scheduler_pick_by=args.scheduler_pick_by,

"""Plot batch-scaling latency and speedup from evaluate outputs.

Run this script from ``experiment/experiment_fig``. Update ``ROOT_DIR`` to a
folder under the repository-level ``output/`` tree that contains
``baseline_compare_*.json`` files generated by ``python src/main.py evaluate``.
...
# python plot_exp1_batch.py
# ============================================================
ROOT_DIR = Path("../../output/evaluate_single_test")
...
def parse_lin_lout(json_path: Path):
    ...
    if not m:
        raise ValueError(f"Cannot parse prefill/decode lengths from filename: {json_path.name}")
    return int(m.group(1)), int(m.group(2))


def parse_model_batch_from_path(json_path: Path):
    """Parse model name and batch size from a run-directory component."""
...
def read_latency(item: dict):
    ...
    raise KeyError(f"No latency field found. Expected one of: {LATENCY_FIELD}, latency, total_time_s")
...
def load_dataframe(root_dir: Path):
    ...
    if not files:
        raise FileNotFoundError(f"No files matching {FILE_GLOB} were found under {root_dir}")
    ...
    for fp in files:
        ...
        if model is None or batch is None:
            raise ValueError(f"Cannot infer model name or batch size from path or config: {fp}")
    ...
    if df.empty:
        raise ValueError("No records were loaded. Check FILE_GLOB, ALGO_ORDER, and LATENCY_FIELD.")
    return df
...
def compute_speedup(df: pd.DataFrame):
    ...
    if SPEEDUP_MODE == "vs_first_batch":
        ...
        raise ValueError("SPEEDUP_MODE must be either 'vs_first_batch' or 'vs_ref_algo'")
...
def compute_speedup_ylim(values: np.ndarray):
    ...
    if SPEEDUP_RANGE_MODE == "tight":
        ...
        raise ValueError("SPEEDUP_RANGE_MODE must be either 'tight' or 'zero_based'")
...
def build_orders(df: pd.DataFrame):
    ...
    if SUBPLOT_ORDER is None:
        ...
        if len(pairs) > SUBPLOTS_PER_ROW:
            print(f"[WARN] Detected {len(pairs)} prefill/decode pairs. Only the first {SUBPLOTS_PER_ROW} pairs will be shown. Set SUBPLOT_ORDER to override this selection.")
...
def plot_grid(df: pd.DataFrame):
    ...
    if not models or not algos or not batches or not pairs:
        raise ValueError("Model, algorithm, batch, or prefill/decode dimensions are missing. Check the input data.")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot batch-scaling latency and speedup from evaluate outputs.")
    parser.add_argument("--root-dir", type=Path, default=ROOT_DIR, help="Directory scanned recursively for baseline_compare_*.json files.")
    parser.add_argument("--save-path", type=str, default=SAVE_PATH, help="Output figure path.")
    args = parser.parse_args()

    globals()["SAVE_PATH"] = str(args.save_path)
    df = load_dataframe(Path(args.root_dir))
    plot_grid(df)

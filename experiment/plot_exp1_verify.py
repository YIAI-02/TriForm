...
def plot_results(dim_results: Dict[Tuple[int, int], Dict[str, Metrics]],
    ...
    for dim in dims:
        ...
        if pd_m and pd_m.trace_total > 0:
            base = pd_m.trace_total
            for m in res.values():
                if m.trace_total and m.trace_total > 0:
                    sp = base / m.trace_total
                    if np.isfinite(sp):
    ...
    for i, ax in enumerate(axes):
        ...
        if pd_m and pd_m.trace_total > 0:
            base = pd_m.trace_total
            speedup = np.array([base / res[a].trace_total if res[a].trace_total > 0 else np.nan for a in order], dtype=float)
        else:
            speedup = np.array([np.nan] * n, dtype=float)
        ...
        if i == min(len(dims), 4) - 1:
            ax2.set_ylabel("Speedup (trace_total, vs pd)")
        else:
            ax2.set_yticks([])
            ax2.set_ylabel("")
    ...
    handles = [
        ...
        Line2D([0], [0], color="k", lw=border_lw * 0.8, label="speedup (trace, vs pd)"),
    ]
    fig.legend(handles=handles, loc="upper center", ncol=5, frameon=False, bbox_to_anchor=(0.5, 1.12))
